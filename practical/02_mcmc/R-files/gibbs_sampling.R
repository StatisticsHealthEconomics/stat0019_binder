# This can be used to allow for variable dates and other variables in the YAML
library(tidyverse)
...
# Plots using tidyverse
tibble(iter=1:nsim,mu=mu,sigma=sigma) %>% ggplot(aes(mu)) + geom_histogram(binwidth = .5)
tibble(x=hist(mu)$mids,y=hist(mu)$counts) %>% ggplot(aes(x,y))+geom_bar(stat="identity")
tibble(iter=1:nsim,mu=mu,sigma=sigma) %>% ggplot(aes(sigma)) + geom_histogram(binwidth = .5) +
  theme_bw()
tibble(x=hist(sigma)$mids,y=hist(sigma)$counts) %>% ggplot(aes(x,y))+geom_bar(stat="identity")
tibble(iter=1:nsim,mu=mu,sigma=sigma) %>% ggplot(aes(iter,mu)) + geom_line() + 
  labs(title="Traceplot for mu")
tibble(iter=1:nsim,mu=mu,sigma=sigma) %>% ggplot(aes(iter,sigma)) + 
...
all <- expand.grid(mu=x1, sigma=x2)
...
tibble(iter=1:nsim,mu=mu,sigma=sigma) %>% mutate(xend=lag(mu),yend=lag(sigma)) %>% 
  ...
  geom_segment(aes(x=xend,y=yend,xend=mu,yend=yend), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed"), linetype=1,col="grey") + 
  geom_segment(aes(x=mu,y=yend,xend=mu,yend=sigma), arrow = arrow(length=unit(0.2,"cm"), ends="last", type = "closed"), linetype=1,col="grey") + 
  geom_point(col="black",size=1.2) + 
  ggrepel::geom_text_repel(aes(label=iter-1),size=5) + 
  theme_bw() + labs(title="Gibbs sampling moves")
